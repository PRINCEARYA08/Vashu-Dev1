  ...
  useEffect(() => {
    ...
    if (window.Chart) {
      ...
      if (ctx) {
        ...
        const statisticsChart = new window.Chart(chartCtx, {
          type: "line",
          data: {
            labels: [
              "Jan",
        ...
        if (myLegendContainer) {
          ...
          for (var i = 0; i < legendItems.length; i += 1) {
            legendItems[i].addEventListener("click", function () {}, false);
          }
        }
      }
    }
    ...
    if (window.Chart) {
      ...
      if (dailySalesChart) {
        const chartCtx = dailySalesChart.getContext("2d");
    ...
    // Sparkline
    if (window.$ && window.$.fn && window.$.fn.sparkline) {
      window.$("#activeUsersChart").sparkline(
